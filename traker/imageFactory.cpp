...
ImageFactory::~ImageFactory() {
  ...
  //TASK 2: convert ranged for loops to while loops
  std::map<std::string, SDL_Surface*>::const_iterator si = surfaces.cbegin();
  while (si != surfaces.cend()) {
    SDL_FreeSurface(si->second);
    ++ si;
  }
  std::map<std::string, SDL_Texture*>::const_iterator ti = textures.cbegin();
  while (ti != textures.cend()) {
    SDL_DestroyTexture(ti->second);
    ++ ti;
  }
  std::map<std::string, Image*>::const_iterator fi = images.cbegin();
  while (fi != images.cend()) {
